...
function saveToHistory(query) {
  ...
  const exists = history.find(h => h.query.toLowerCase() === query.toLowerCase());
  if (exists) return;
  const updated = [{ query, date: new Date().toISOString() }, ...history].slice(0, 20);
...
export default function ResultsPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError('');
    saveToHistory(query); // ← Save to search history
  ...
  const filtered = (inStockOnly ? results.filter(r => r.inStock) : results)
    .sort((a, b) => sortBy === 'price' ? a.price - b.price : a.distance - b.distance);

  const inStockResults = filtered.filter(r => r.inStock);
  const minPrice = inStockResults.length ? Math.min(...inStockResults.map(r => r.price)) : 0;
  ...
  // Save pharmacy to saved list
  const savePharmacy = (result) => {
    const saved = JSON.parse(localStorage.getItem('medimap_saved_pharmacies') || '[]');
    const exists = saved.find(p => p.id === result.pharmacy._id);
    if (exists) return;
    const updated = [{
      id: result.pharmacy._id || `p${Date.now()}`,
      name: result.pharmacy.name,
      address: result.pharmacy.address,
      rating: result.pharmacy.rating,
      isOpen: result.pharmacy.isOpen,
  ...
  // Save money tracker
  const trackSaving = (result) => {
    if (!minPrice || result.price !== minPrice) return;
    const savings = JSON.parse(localStorage.getItem('medimap_saved_money') || JSON.stringify({ total: 0, thisMonth: 0, transactions: [] }));
    const saved = avgPrice - minPrice;
    if (saved <= 0) return;
    const updated = {
      total: savings.total + saved,
      thisMonth: savings.thisMonth + saved,
      transactions: [{
        medicine: query,
        saved,
        ...
        cheapest: minPrice,
        avg: avgPrice
      }, ...savings.transactions].slice(0, 20)
    };
    localStorage.setItem('medimap_saved_money', JSON.stringify(updated));
  };
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div>
          ...
          {!loading && (
            <p className="text-sm text-gray-500 mt-0.5">{filtered.length} {t('pharmaciesFound')}</p>
          )}
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <label className="flex items-center gap-2 text-sm cursor-pointer">
            <input type="checkbox" checked={inStockOnly} onChange={e => setInStockOnly(e.target.checked)} className="w-4 h-4 accent-[#2E7DFF]" />
        ...
        <>
          ...
          <div className="space-y-3 mt-4">
            {filtered.map((result, i) => (
              <div key={result._id || i} className="relative">
                <PharmacyCard result={result} />
                {/* Save button overlay */}
                <button
                  onClick={() => { savePharmacy(result); trackSaving(result); }}
                  className="absolute top-3 right-3 text-xs text-gray-400 hover:text-[#2E7DFF] transition-colors"
                  title="Save pharmacy"
                >
                  🔖
                </button>
          ...
          {filtered.length === 0 && (
            <div className="card p-10 text-center">
              ...
              <Link to="/" className="btn-primary mt-4 inline-block text-sm">← {t('backToHome')}</Link>
            </div>
          )}
          ...
          {filtered.length > 0 && (
            <div className="mt-8 card p-5 bg-blue-50 border border-blue-100 text-center">
              ...
              <Link to="/submit-price" className="btn-primary text-sm inline-block">{t('submitPriceUpdate')}</Link>
            </div>
          )}
        </>
      )}
